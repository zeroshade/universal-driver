...
TEST_CASE("should fail converting TIMESTAMP_NTZ to numeric C types", "[timestamp_ntz][conversion][negative]") {
  // Given Snowflake client is logged in
  Connection conn;

  // When Query "SELECT '2024-01-15 14:30:45'::TIMESTAMP_NTZ" is executed
...
TEST_CASE("should fail converting TIMESTAMP_NTZ to additional numeric C types",
          "[timestamp_ntz][conversion][negative]") {
  // Given Snowflake client is logged in
  Connection conn;
...
TEST_CASE("should fail converting TIMESTAMP_NTZ to single-component interval C types",
          "[timestamp_ntz][conversion][negative]") {
  // Given Snowflake client is logged in
  Connection conn;
...
TEST_CASE("should fail converting TIMESTAMP_NTZ to compound interval C types",
          "[timestamp_ntz][conversion][negative]") {
  // Given Snowflake client is logged in
  Connection conn;
...
TEST_CASE("should fail converting TIMESTAMP_NTZ to SQL_C_GUID", "[timestamp_ntz][conversion][negative]") {
  // Given Snowflake client is logged in
  Connection conn;
