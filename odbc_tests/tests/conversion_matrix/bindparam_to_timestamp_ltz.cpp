...
// clang-format off
static const SqlTypeInfo TIMESTAMP_LTZ_SQL_TYPES[] = {
...
TEST_CASE("conversion matrix: all C types -> TIMESTAMP_LTZ column via SQLBindParameter",
  ...
  for (const auto& st : TIMESTAMP_LTZ_SQL_TYPES) {
    for (const auto& ct : ALL_C_TYPES) {
      try_bindparam(conn, "INSERT INTO cm_timestamp_ltz VALUES (?)", ct, st, report);
    }
  }
}
