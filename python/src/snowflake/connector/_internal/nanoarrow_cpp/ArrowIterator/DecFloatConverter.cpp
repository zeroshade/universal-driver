...
Logger* DecFloatConverter::logger = new Logger("snowflake.connector.DecFloatConverter");
...
PyObject* DecFloatConverter::toPyObject(int64_t rowIndex) const {
  ...
  ArrowStringView stringView = ArrowArrayViewGetStringUnsafe(m_significand, rowIndex);
