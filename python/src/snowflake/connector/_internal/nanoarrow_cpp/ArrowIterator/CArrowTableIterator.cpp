...
void CArrowTableIterator::convertIfNeeded(ArrowSchema* columnSchema, ArrowArrayView* columnArray) {
  ...
  switch (st) {
    case SnowflakeType::Type::FIXED: {
      ...
      if (scale > 0 && columnSchemaView.type != ArrowType::NANOARROW_TYPE_DECIMAL128) {
        ...
        convertScaledFixedNumberColumn_nanoarrow(&columnSchemaView, columnArray, scale);
      }
      break;
    }
      ...
      switch (columnSchemaView.type) {
        ...
        default: {
          std::string errorInfo = Logger::formatString(
              ...
              "for ARRAY data in %s",
              NANOARROW_TYPE_ENUM_STRING[columnSchemaView.type], columnSchemaView.schema->name);
          logger->error(__FILE__, __func__, __LINE__, errorInfo.c_str());
          PyErr_SetString(PyExc_Exception, errorInfo.c_str());
          break;
        }
    ...
    case SnowflakeType::Type::INTERVAL_DAY_TIME: {
      ...
      convertIntervalDayTimeColumn_nanoarrow(&columnSchemaView, columnArray, scale);
      break;
    }
    ...
    case SnowflakeType::Type::TIMESTAMP_LTZ: {
      ...
      convertTimestampColumn_nanoarrow(&columnSchemaView, columnArray, scale, m_timezone);
      break;
    }
...
void CArrowTableIterator::convertScaledFixedNumberColumn_nanoarrow(ArrowSchemaView* field,
  ...
  if (m_convert_number_to_decimal) {
    convertScaledFixedNumberColumnToDecimalColumn_nanoarrow(field, columnArray, scale);
  } else {
    convertScaledFixedNumberColumnToDoubleColumn_nanoarrow(field, columnArray, scale);
  }
}
...
void CArrowTableIterator::convertTimestampColumn_nanoarrow(ArrowSchemaView* field,
  ...
  if (!m_force_microsecond_precision && scale > 6 && field->type == NANOARROW_TYPE_STRUCT) {
...
void CArrowTableIterator::convertTimestampTZColumn_nanoarrow(ArrowSchemaView* field,
  ...
  if (scale == 0) {
    ...
    timeunit = has_overflow_to_downscale ? NANOARROW_TIME_UNIT_MICRO : NANOARROW_TIME_UNIT_NANO;
  }
    ...
    if (!ArrowArrayViewIsNull(columnArray, rowIdx)) {
      if (byteLength == 8) {
        ...
        if (scale == 0) {
          ...
          if (has_overflow_to_downscale) {
            returnCode = ArrowArrayAppendInt(newArray, epoch * sf::internal::powTenSB4[6]);
          } else {
