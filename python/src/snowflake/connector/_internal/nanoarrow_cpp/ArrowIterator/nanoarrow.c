...
static void ArrowBufferAllocatorMallocFree(struct ArrowBufferAllocator* allocator, uint8_t* ptr,
                                           int64_t size) {
  ArrowFree(ptr);
}
...
struct ArrowBufferAllocator ArrowBufferAllocatorDefault(void) { return ArrowBufferAllocatorMalloc; }

static uint8_t* ArrowBufferAllocatorNeverReallocate(struct ArrowBufferAllocator* allocator,
                                                    uint8_t* ptr, int64_t old_size,
                                                    int64_t new_size) {
  return NULL;
}
...
struct ArrowBufferAllocator ArrowBufferDeallocator(
    void (*custom_free)(struct ArrowBufferAllocator* allocator, uint8_t* ptr, int64_t size),
    void* private_data) {
...
ArrowErrorCode ArrowSchemaSetTypeFixedSize(struct ArrowSchema* schema, enum ArrowType type,
                                           int32_t fixed_size) {
  if (fixed_size <= 0) {
    return EINVAL;
  }
...
static ArrowErrorCode ArrowSchemaViewParse(struct ArrowSchemaView* schema_view, const char* format,
                                           const char** format_end_out, struct ArrowError* error) {
  ...
  switch (format[0]) {
    ...
    case 'd':
      if (format[1] != ':' || format[2] == '\0') {
        ArrowErrorSet(error, "Expected ':precision,scale[,bitwidth]' following 'd'", format + 3);
        return EINVAL;
      }
      ...
      switch (format[1]) {
        ...
        case 't':
          switch (format[2]) {
            ...
            default:
              ArrowErrorSet(error,
                            "Expected 's', 'm', 'u', or 'n' following 'tt' but "
                            "found '%s'",
                            format + 2);
              return EINVAL;
          }
        ...
        case 's':
          switch (format[2]) {
            ...
            default:
              ArrowErrorSet(error,
                            "Expected 's', 'm', 'u', or 'n' following 'ts' but "
                            "found '%s'",
                            format + 2);
              return EINVAL;
          }
          ...
          switch (format[2]) {
            ...
            case 'n':
              ...
              *format_end_out = format + 3;
              return NANOARROW_OK;
            default:
              ArrowErrorSet(error, "Expected 's', 'm', u', or 'n' following 'tD' but found '%s'",
                            format + 2);
              return EINVAL;
          }
          ...
          switch (format[2]) {
            ...
            case 'n':
              ...
              *format_end_out = format + 3;
              return NANOARROW_OK;
            default:
              ArrowErrorSet(error, "Expected 'M', 'D', or 'n' following 'ti' but found '%s'",
                            format + 2);
              return EINVAL;
          }
        ...
        default:
          ArrowErrorSet(error,
                        "Expected 'd', 't', 's', 'D', or 'i' following 't' but "
                        "found '%s'",
                        format + 1);
          return EINVAL;
      }
...
static ArrowErrorCode ArrowSchemaViewValidateNChildren(struct ArrowSchemaView* schema_view,
  ...
  if (n_children != -1 && schema_view->schema->n_children != n_children) {
    ArrowErrorSet(error, "Expected schema with %d children but found %d children", (int)n_children,
                  (int)schema_view->schema->n_children);
    return EINVAL;
  }
  ...
  for (int64_t i = 0; i < schema_view->schema->n_children; i++) {
    ...
    if (child == NULL) {
      ArrowErrorSet(error, "Expected valid schema at schema->children[%d] but found NULL", i);
      return EINVAL;
    } else if (child->release == NULL) {
      ArrowErrorSet(error,
                    "Expected valid schema at schema->children[%d] but found a "
                    "released schema",
                    i);
      return EINVAL;
    }
...
static ArrowErrorCode ArrowSchemaViewValidateDictionary(struct ArrowSchemaView* schema_view,
                                                        struct ArrowError* error) {
  ...
  switch (schema_view->storage_type) {
    ...
    default:
      ArrowErrorSet(error,
                    "Expected dictionary schema index type to be an integral "
                    "type but found '%s'",
...
static ArrowErrorCode ArrowSchemaViewValidate(struct ArrowSchemaView* schema_view,
                                              enum ArrowType type, struct ArrowError* error) {
  switch (type) {
    case NANOARROW_TYPE_NA:
    case NANOARROW_TYPE_BOOL:
    case NANOARROW_TYPE_UINT8:
...
ArrowErrorCode ArrowSchemaViewInit(struct ArrowSchemaView* schema_view, struct ArrowSchema* schema,
  ...
  if (format == NULL) {
    ArrowErrorSet(error,
                  "Error parsing schema->format: Expected a null-terminated "
                  "string but found NULL");
    return EINVAL;
  }
  ...
  if ((format + format_len) != format_end_out) {
    ArrowErrorSet(error, "Error parsing schema->format '%s': parsed %d/%d characters", format,
                  (int)(format_end_out - format), (int)(format_len));
    return EINVAL;
  }
...
static int64_t ArrowSchemaTypeToStringInternal(struct ArrowSchemaView* schema_view, char* out,
                                               int64_t n) {
  const char* type_string = ArrowTypeString(schema_view->type);
  switch (schema_view->type) {
    case NANOARROW_TYPE_DECIMAL128:
    case NANOARROW_TYPE_DECIMAL256:
      return snprintf(out, n, "%s(%d, %d)", type_string, (int)schema_view->decimal_precision,
...
static inline void ArrowToStringLogChars(char** out, int64_t n_chars_last, int64_t* n_remaining,
                                         int64_t* n_chars) {
...
int64_t ArrowSchemaToString(struct ArrowSchema* schema, char* out, int64_t n, char recursive) {
  if (schema == NULL) {
  ...
  if (is_extension && is_dictionary) {
    n_chars_last =
        snprintf(out, n, "%.*s{dictionary(%s)<", (int)schema_view.extension_name.size_bytes,
  ...
  if (recursive && schema->format[0] == '+') {
    ...
    for (int64_t i = 0; i < schema->n_children; i++) {
      ...
      // error, but we need the name first
...
static ArrowErrorCode ArrowMetadataBuilderAppendInternal(struct ArrowBuffer* buffer,
  ...
  NANOARROW_RETURN_NOT_OK(
      ArrowBufferReserve(buffer, sizeof(int32_t) + key_size + sizeof(int32_t) + value_size));
...
static ArrowErrorCode ArrowMetadataBuilderSetInternal(struct ArrowBuffer* buffer,
  ...
  while (reader.remaining_keys > 0) {
    ...
    if (key->size_bytes == existing_key.size_bytes &&
        strncmp((const char*)key->data, (const char*)existing_key.data, existing_key.size_bytes) ==
            0) {
...
ArrowErrorCode ArrowMetadataBuilderAppend(struct ArrowBuffer* buffer, struct ArrowStringView key,
                                          struct ArrowStringView value) {
  return ArrowMetadataBuilderAppendInternal(buffer, &key, &value);
}

ArrowErrorCode ArrowMetadataBuilderSet(struct ArrowBuffer* buffer, struct ArrowStringView key,
                                       struct ArrowStringView value) {
...
ArrowErrorCode ArrowArrayInitFromArrayView(struct ArrowArray* array,
                                           ...
                                           struct ArrowError* error) {
  NANOARROW_RETURN_NOT_OK_WITH_ERROR(ArrowArrayInitFromType(array, array_view->storage_type),
                                     error);
  int result;
...
ArrowErrorCode ArrowArrayInitFromSchema(struct ArrowArray* array, struct ArrowSchema* schema,
                                        struct ArrowError* error) {
  struct ArrowArrayView array_view;
...
ArrowErrorCode ArrowArrayFinishBuilding(struct ArrowArray* array,
  ...
  // non-null in some implementations (at least one version of Arrow C++ at the
  // time this was added). Only do this fix if we can assume CPU data access.
  ...
  struct ArrowArrayView array_view;
  NANOARROW_RETURN_NOT_OK_WITH_ERROR(ArrowArrayViewInitFromArray(&array_view, array), error);
...
ArrowErrorCode ArrowArrayViewInitFromSchema(struct ArrowArrayView* array_view,
  ...
  if (array_view->storage_type == NANOARROW_TYPE_SPARSE_UNION ||
    ...
    int8_t n_type_ids =
        _ArrowParseUnionTypeIds(schema_view.union_type_ids, array_view->union_type_id_map + 128);
...
static int ArrowArrayViewSetArrayInternal(struct ArrowArrayView* array_view,
  ...
  for (int64_t i = 0; i < array_view->n_children; i++) {
    NANOARROW_RETURN_NOT_OK(
        ArrowArrayViewSetArrayInternal(array_view->children[i], array->children[i], error));
  }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    switch (array_view->layout.buffer_type[i]) {
      ...
      case NANOARROW_BUFFER_TYPE_DATA_OFFSET:
        ...
        break;
      case NANOARROW_BUFFER_TYPE_DATA:
        min_buffer_size_bytes = _ArrowRoundUpToMultipleOf8(array_view->layout.element_size_bits[i] *
                                                           offset_plus_length) /
                                8;
        break;
      case NANOARROW_BUFFER_TYPE_TYPE_ID:
    ...
    if (array_view->buffer_views[i].size_bytes == -1) {
      ...
      ArrowErrorSet(error,
                    ...
                    ArrowTypeString(array_view->storage_type), (int)i, (long)min_buffer_size_bytes,
    ...
    case NANOARROW_TYPE_STRUCT:
      ...
      for (int64_t i = 0; i < array_view->n_children; i++) {
        if (array_view->children[i]->length < child_min_length) {
          ArrowErrorSet(error,
                        "Expected struct child %d to have length >= %ld but "
                        "found child with "
                        "length %ld",
  ...
  switch (array_view->storage_type) {
    ...
    case NANOARROW_TYPE_BINARY:
      if (array_view->buffer_views[1].size_bytes != 0) {
        ...
        if (first_offset < 0) {
          ArrowErrorSet(error, "Expected first offset >= 0 but found %ld", (long)first_offset);
          return EINVAL;
        }
    ...
    case NANOARROW_TYPE_MAP:
      if (array_view->buffer_views[1].size_bytes != 0) {
        ...
        if (array_view->children[0]->length < last_offset) {
          ArrowErrorSet(error,
                        "Expected child of %s array to have length >= %ld but "
                        "found array with "
                        "length %ld",
                        ArrowTypeString(array_view->storage_type), (long)last_offset,
...
ArrowErrorCode ArrowArrayViewSetArray(struct ArrowArrayView* array_view, struct ArrowArray* array,
                                      struct ArrowError* error) {
  ...
  // Run default validation. Because we've marked all non-NULL buffers as having
  // unknown size, validation will also update the buffer sizes as it goes.
...
static int ArrowAssertRangeInt8(struct ArrowBufferView view, int8_t min_value, int8_t max_value,
                                struct ArrowError* error) {
  for (int64_t i = 0; i < view.size_bytes; i++) {
    if (view.data.as_int8[i] < min_value || view.data.as_int8[i] > max_value) {
      ArrowErrorSet(error, "[%ld] Expected buffer value between %d and %d but found value %d",
...
static int ArrowAssertInt8In(struct ArrowBufferView view, const int8_t* values, int64_t n_values,
                             struct ArrowError* error) {
  for (int64_t i = 0; i < view.size_bytes; i++) {
    int item_found = 0;
    ...
    if (!item_found) {
      ArrowErrorSet(error, "[%ld] Unexpected buffer value %d", (long)i, (int)view.data.as_int8[i]);
      return EINVAL;
    }
  }
...
static int ArrowArrayViewValidateFull(struct ArrowArrayView* array_view, struct ArrowError* error) {
    ...
    switch (array_view->layout.buffer_type[i]) {
      case NANOARROW_BUFFER_TYPE_DATA_OFFSET:
        if (array_view->layout.element_size_bits[i] == 32) {
          NANOARROW_RETURN_NOT_OK(ArrowAssertIncreasingInt32(array_view->buffer_views[i], error));
        } else {
          NANOARROW_RETURN_NOT_OK(ArrowAssertIncreasingInt64(array_view->buffer_views[i], error));
        }
        break;
      default:
        break;
  ...
  if (array_view->storage_type == NANOARROW_TYPE_DENSE_UNION ||
    ...
    if (array_view->union_type_id_map == NULL) {
      ...
      // ArrowArrayInitFromType() + ArrowArrayAllocateChildren() +
      ...
      return EINVAL;
                   ...
                   array_view->union_type_id_map, array_view->n_children, array_view->n_children)) {
  ...
  if (array_view->storage_type == NANOARROW_TYPE_DENSE_UNION &&
    ...
    for (int64_t i = 0; i < array_view->length; i++) {
      ...
      if (offset < 0 || offset > child_length) {
        ArrowErrorSet(error,
                      "[%ld] Expected union offset for child id %d to be "
                      "between 0 and %ld but "
