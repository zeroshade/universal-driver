 ...
 * to fail.
 */
typedef int flatcc_builder_emit_fun(void* emit_context, const flatcc_iovec_t* iov, int iov_count,
 ...
 * success.
 */
int flatcc_builder_custom_reset(flatcc_builder_t* B, int reduce_buffers, int set_defaults);
...
#define FLATCC_BUILDER_INIT_VT_HASH(hash) \
  {                                       \
    (hash) = (uint32_t)0x2f693b52UL;      \
  }
#endif
