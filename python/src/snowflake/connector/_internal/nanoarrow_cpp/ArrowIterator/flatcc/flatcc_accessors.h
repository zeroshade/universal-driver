...
#define __flatcc_basic_scalar_accessors_impl(N, T, W, E)                               \
  ...
  static inline T* N##__ptr_add(T* p, size_t i) { return p + i; }                      \
  static inline const T* N##__const_ptr_add(const T* p, size_t i) { return p + i; }    \
  static inline T N##_read_from_pe(const void* p) { return N##_cast_from_pe(*(T*)p); } \
  static inline T N##_read_to_pe(const void* p) { return N##_cast_to_pe(*(T*)p); }     \
  static inline T N##_read(const void* p) { return *(T*)p; }                           \
  static inline void N##_write_from_pe(void* p, T v) { *(T*)p = N##_cast_from_pe(v); } \
  static inline void N##_write_to_pe(void* p, T v) { *(T*)p = N##_cast_to_pe(v); }     \
  static inline void N##_write(void* p, T v) { *(T*)p = v; }                           \
  static inline T N##_read_from_le(const void* p) { return N##_cast_from_le(*(T*)p); } \
  typedef struct {                                                                     \
    int is_null;                                                                       \
    T value;                                                                           \
  } N##_option_t;
...
#define __flatcc_define_integer_accessors_impl(N, T, W, E)                        \
  static inline T N##_cast_from_pe(T v) { return (T)E##W##toh((uint##W##_t)v); }  \
  static inline T N##_cast_to_pe(T v) { return (T)hto##E##W((uint##W##_t)v); }    \
  static inline T N##_cast_from_le(T v) { return (T)le##W##toh((uint##W##_t)v); } \
  static inline T N##_cast_to_le(T v) { return (T)htole##W((uint##W##_t)v); }     \
  static inline T N##_cast_from_be(T v) { return (T)be##W##toh((uint##W##_t)v); } \
  static inline T N##_cast_to_be(T v) { return (T)htobe##W((uint##W##_t)v); }     \
