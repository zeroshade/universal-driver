...
MapConverter::MapConverter(ArrowSchemaView* schemaView, ArrowArrayView* array, PyObject* context,
  ...
  m_key_converter = getConverterFromSchema(key_schema, key_array, context, useNumpy, logger);

  ArrowSchema* value_schema = entries->children[1];
  ArrowArrayView* value_array = array->children[0]->children[1];
  m_value_converter = getConverterFromSchema(value_schema, value_array, context, useNumpy, logger);
}
...
PyObject* MapConverter::toPyObject(int64_t rowIndex) const {
  ...
  for (int i = start; i < end; i++) {
    PyDict_SetItem(dict, m_key_converter->toPyObject(i), m_value_converter->toPyObject(i));
  }
  return dict;
}
