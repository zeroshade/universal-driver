...
#define NANOARROW_THROW_NOT_OK(EXPR) \
  _NANOARROW_THROW_NOT_OK_IMPL(_NANOARROW_MAKE_NAME(errno_status_, __COUNTER__), EXPR, #EXPR)
...
static inline void init_pointer(struct ArrowArrayStream* data) { data->release = nullptr; }
...
static inline void release_pointer(struct ArrowArrayView* data) { ArrowArrayViewReset(data); }
...
class EmptyArrayStream {
 ...
 protected:
  ...
  EmptyArrayStream(struct ArrowSchema* schema) : schema_(schema) { error_.message[0] = '\0'; }
 ...
 private:
  static int get_schema_wrapper(struct ArrowArrayStream* stream, struct ArrowSchema* schema) {
...
class VectorArrayStream : public EmptyArrayStream {
 public:
  ...
  static UniqueArrayStream MakeUnique(struct ArrowSchema* schema, struct ArrowArray* array) {
  ...
  static UniqueArrayStream MakeUnique(struct ArrowSchema* schema, std::vector<UniqueArray> arrays) {
    UniqueArrayStream stream;
