...
class CArrowTableIterator : public CArrowIterator {
 ...
 private:
   ...
   * setting
   */
  void convertScaledFixedNumberColumn_nanoarrow(ArrowSchemaView* field, ArrowArrayView* columnArray,
   ...
   */
  template <typename T>
  double convertScaledFixedNumberToDouble(const unsigned int scale, T originalValue);
};
}  // namespace sf
