...
namespace sf {
Logger* ArrayConverter::logger = new Logger("snowflake.connector.ArrayConverter");
...
ArrayConverter::ArrayConverter(ArrowSchemaView* schemaView, ArrowArrayView* array,
  ...
  m_item_converter = getConverterFromSchema(item_schema, item_array, context, useNumpy, logger);
}
